...
#include <esp_mac.h>
#include <esp_random.h>            // esp_efuse_mac_get_default (IDF 5)
...
void AppConfig::load() {
  ...
  p.end();
  // no nonce yet (fresh flash with erase, factory reset, unbond): make one and keep
  // it even while the device is otherwise unconfigured, so its address stays put
  // between the boots of the setup phase
  if (bleNonce == 0) {
    while (bleNonce == 0) bleNonce = esp_random();
    Preferences w;
    w.begin(NVS_NS, false);
    w.putUInt("blenonce", bleNonce);
    w.end();
  }
}

void AppConfig::renewBleAddress() {
  Preferences w;
  w.begin(NVS_NS, false);
  w.remove("blenonce");                 // regenerated by load() at the next boot
  w.end();
}
...
void AppConfig::save() {
  ...
  chk(p.putUInt("blenonce", bleNonce));
